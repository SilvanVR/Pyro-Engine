...
layout (set = 1, binding = 3) uniform LightCounts_S
{
...
layout (set = 1, binding = 4) uniform Lighting_S
{
	vec3 ambientLight;
...
layout (location = 5) in vec4 inShadowCoords[MAX_NUM_SHADOW_MAPS_DIR_AND_SPOT];
...
{	
	...
	outColor = texture(diffuse, texCoords) * material.color * vec4(totalLight, 1.0);
	
	if(renderNormalMaps > 0)
		outColor = texture(normalMap, inUV * material.uvScale);
		
	if(renderDispMaps > 0)
		outColor = texture(dispMap, inUV * material.uvScale);
