...
{		
    ...
    for(uint i = 0u; i < ubo.numSamples; ++i)
    {
        ...
        vec2 Xi = Hammersley(i, ubo.numSamples);
        ...
        if(NdotL > 0.0)
        {
            ...
            float saTexel  = 4.0 * PI / (6.0 * ubo.resolution * ubo.resolution);
            float saSample = 1.0 / (float(ubo.numSamples) * pdf + 0.0001);

            float mipLevel = ubo.roughness == 0.0 ? 0.0 : 0.5 * log2(saSample / saTexel); 
            
            prefilteredColor += textureLod(CubemapIn_S, L, mipLevel).rgb * NdotL;
            totalWeight      += NdotL;
        }
    }
